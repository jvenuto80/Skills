...
**Procedure.** Before you invent, ask whose problem this is. A problem that is *yours specifically* — your code, your config, your data — you build for; no one else has your bug. A problem that is *everyone's* — an upstream break, a common error string, a widely-used tool that changed under you — someone has almost certainly hit before you, and the cheapest correct move is to find them. The rule: cheap search runs alongside your own investigation from the first minute — the issue tracker, the changelog, the commit that broke it, the thread of strangers with your exact symptom — while the expensive work — reverse-engineering, decompiling, packet capture — waits until those searches come back empty. Primary sources are the last resort that feels like the first instinct. And when your own approach hits a hard wall, the wall is not a verdict that the goal is impossible — it is the signal to go find whoever located the side door.

**Example.** A vendor deprecates their login API and every open-source client breaks at once. The engineering path — capture the app's traffic, decompile it, discover the login now runs through a native anti-tamper SDK — is real, correct, and dead: un-replicable. Conclusion drawn: "wait for upstream." But the *same* decompiled code held a second login endpoint that never touched the SDK, and a stranger on the issue tracker had already wired it into a working flow — posted before the reverse-engineering began, found only when someone else pointed to it. Two minutes of reading the tracker would have replaced two days of out-engineering it.
